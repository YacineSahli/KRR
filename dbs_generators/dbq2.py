...
"""q3(z,w):=E x, y, v | r1(x | y, z) et r2(y | v, w)
"""
...
def generateYesInstance(size, inconsistency):
    ...
    """ for i in range(size):
        ...
        if r<inconsistency:
            ...
            res += "r2(y"+str(i)+",v"+str(size+i)+",w"+str(size+i)+").\n"
    return res """
...
def generateNoInstance(size, inconsistency, error):
    ...
    for i in range(x2-1):
        ...
        if i < x32:
            ...
            res += "r2(v"+str(r2)+",u"+str(x1+i+size)+",d"+str(x1+i)+").\n"
    return res
